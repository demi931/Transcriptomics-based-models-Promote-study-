...
# Read raw count data
count_raw <- read.csv(expr_file, row.names = 1, check.names = FALSE)

# Filter out genes with zero counts across all samples
count_filtered <- count_raw[rowSums(count_raw) != 0, ]

# Read sample metadata
sample_metadata <- read.csv("data/input/PROMOTE2.1.csv", stringsAsFactors = TRUE)
...
head(deseq_results_df)

# Add a 'group' column based on log2FoldChange and adjusted p-value
...
table(deseq_results_df$group)

# Write results to CSV
write.csv(deseq_results_df, file = "diff_expr_result.csv", quote = FALSE)
```
...
foldchange_data <- read.csv("data/input/foldchangeJ.csv", row.names = 1)

# Apply log2 transformation
log2_foldchange <- log2(foldchange_data + 1)

# Save log-transformed data
write.csv(log2_foldchange, file = "log2J.csv", row.names = TRUE)
...
```{r}
# Read fold-change data for RR
rr_foldchange_data <- read.csv("data/input/RRfoldchangeJ.csv", row.names = 1)

# Apply log2 transformation
log2_rr_foldchange <- log2(rr_foldchange_data + 1)

# Save log-transformed data
write.csv(log2_rr_foldchange, file = "log2JRR(1).csv", row.names = TRUE)

#Read the log-transformed data from the "log2JRR(1).csv" file, using the first column as row names

expr_rr_log2 <- read.csv("data/input/log2JRR(1).csv", row.names = 1)

# Extract sample-level group information
rr_group_vector <- as.factor(unlist(expr_rr_log2["group", ]))

# Remove 'group' row to get expression matrix only
expr_rr_matrix <- expr_rr_log2[rownames(expr_rr_log2) != "group", ]

expr_rr_numeric <- data.matrix(expr_rr_matrix)

# Build column annotation data frame
annotation_rr <- data.frame(
  Group = ifelse(rr_group_vector == "1", "Response", "No response")
)
rownames(annotation_rr) <- colnames(expr_rr_numeric)
...
expr_data <- read.csv("data/input/20260128stagepca(1).csv",row.names = 1)
# Extract sample annotations
...
pcdat <- prcomp(expr_mat_t, scale. = TRUE)
summary(pcdat)
...
expr_raw <- read.csv("data/input/20251218selectedcbr+5clinic.csv",row.names = 1)

# Define conditions and group labels
group <- as.numeric(expr_raw[1, ])
...
# Transpose the data
expr_raw <- expr_raw[-c(1), ]

expr_matrix <- as.data.frame(t(expr_raw))

# Set row names for the group data frame
rownames(df_group) <- rownames(expr_matrix)

# Combine conditions and data for ordering
y_condition <- rbind(t(df_group), as.data.frame(t(expr_matrix)))
...
expr_raw <- read.csv("data/input/20251218selectedRR+5clinic.csv",row.names = 1)
...
# Remove group row and transpose expression data
expr_raw <- expr_raw[-c(1), ]
